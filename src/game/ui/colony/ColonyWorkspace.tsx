...
import { resolveLayoutSelection, type LayoutMode } from './layoutModeSelection';
...
export function ColonyWorkspace() {
  ...
  // /colony ÜRETİLEN tasarımla açılır; Faz 3 sabit yerleşimi "Default" düğmesi olarak kalır.
  const [layoutMode, setLayoutMode] = useState<LayoutMode>('generated');
  ...
  // Üretim boş/hatalı dönerse ÇÖKME YOK: Default'a güvenli geri düşüş (bkz. resolveLayoutSelection).
  const { candidates, effectiveMode, generationFallback, selectedLayout } = resolveLayoutSelection(generation, layoutMode, selectedCandidateIndex);
  ...
  return (
    <section aria-labelledby="colony-heading" className="workspace colony-prototype">
      ...
      <div className="prototype-stage">
        <WorldScene cameraResetToken={cameraResetToken} debugPanelOpen={debugPanelOpen} debugState={debugState} layout={selectedLayout} onMetrics={setMetrics} onObjectInspection={setObjectInspection} usePrototypeLayout={effectiveMode === 'prototype'} />
        <PrototypeDebugPanel candidates={candidates} inspection={objectInspection} layoutMode={effectiveMode} metrics={metrics} onCameraPreset={setCameraPreset} onChange={setDebugState} onLayoutModeChange={setLayoutMode} onNewLayoutSeed={() => { setLayoutSeed((seed) => seed + 1); setSelectedCandidateIndex(0); setCameraResetToken((token) => token + 1); }} onSelectCandidate={(index) => { setSelectedCandidateIndex(index); setLayoutMode('generated'); setCameraResetToken((token) => token + 1); }} onToggle={() => setDebugPanelOpen((open) => !open)} open={debugPanelOpen} seedSweep={seedSweep} selectedCandidateIndex={selectedCandidateIndex} state={debugState} />
        {generationFallback && <p className="layout-generation-fallback" role="status">{t('layoutReview.generationFallback')}</p>}
