...
describe('Faz 4 deterministic layout generator', () => {
  ...
  it('44. candidate order seed-based rotation ile deterministic', () => { const dup = generateLayoutCandidates({ seed: result.seed }); expect(dup.status === 'success' && result.status === 'success' ? dup.candidates.map(c => c.candidateId).join() : '').toEqual(result.status === 'success' ? result.candidates.map(c => c.candidateId).join() : ''); });
